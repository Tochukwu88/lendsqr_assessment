...
const { User, Wallet, Transaction, BankDetails, } = require('../models/index');
const Paystack = require('../utils/paystack');
const { internalResponse } = require('../utils/responseHandler');

class Wallets {
    static async createWallet(id) {
        let transaction
        try {
           
            transaction = await db.sequelize.transaction()
            
                const wallet = await Wallet.findOne({ where: { userId: id } }, { transaction })
                
                if (!wallet) {

                  const wallets =  await Wallet.create({ userId: id, balance: 0.00 }, { transaction })
                  console.log(wallets)
